...
在 Firefox 实现 Wayland 支持后，`xprop`/`xwininfo` 将无法获取 `WM_Class` 值，而目前也没有官方文档标明了这些信息，如 Gist[^gh] 中评论所说，`WM_Class` 的值可能会变更，后续 Firefox 默认使用 Wayland 时可能需要另外的操作，也希望 Mozilla 之后能够增加 desktop 相关的说明。
